...
from eq_prediction.preprocessing.data_preprocessing import Data_Sets, EQDataLoader, DataPreprocessor, DataEncoder, ZScoreStandard, ExperimentalDataPreprocessor
...
REGRESSION_DATA_PATH = PROJECT_ROOT / "data" / "model_ready" / "earthquakes_model_ready.csv"
...
class DTRegressor:
    ...
    def _split_xy(self, dataframe):
        if "mag" not in dataframe.columns:
            raise ValueError("DTRegressor data must include a 'mag' target column.")
        X_set = dataframe.drop(columns=["mag"])
        y_set = dataframe["mag"]
        return X_set, y_set

    def _load_pipeline_data(self, source, fetch_new, enrich_details, save):
        run_pipeline(
            source=source,
            fetch_new=fetch_new,
            enrich_details=enrich_details,
        ...
        valid_df = load_dataset("validation")
        test_df = load_dataset("test")

        train_X, train_y = self._split_xy(train_df)
        valid_X, valid_y = self._split_xy(valid_df)
        test_X, test_y = self._split_xy(test_df)
        return {
            "X_train": train_X,
            "y_train": train_y,
            "X_valid": valid_X,
    ...
    def _prepare_data(self, data, source, fetch_new, enrich_details, save):
        if data is None:
            return self._load_pipeline_data(source, fetch_new, enrich_details, save)

        if isinstance(data, dict):
            expected = {"X_train", "y_train", "X_valid", "y_valid", "X_test", "y_test"}
            if expected.issubset(data):
                return data
            if {"train", "validation", "test"}.issubset(data):
                train_X, train_y = self._split_xy(data["train"])
                valid_X, valid_y = self._split_xy(data["validation"])
                test_X, test_y = self._split_xy(data["test"])
                return {
                    "X_train": train_X,
                    "y_train": train_y,
                    "X_valid": valid_X,
            ...
            raise ValueError(
                "DTRegressor data dict must contain either X/y split keys "
                "or train/validation/test dataframes."
            )

        if isinstance(data, pd.DataFrame):
            train_end = int(len(data) * 0.70)
            validation_end = train_end + int(len(data) * 0.15)
            train_X, train_y = self._split_xy(data.iloc[:train_end].reset_index(drop=True))
            valid_X, valid_y = self._split_xy(data.iloc[train_end:validation_end].reset_index(drop=True))
            test_X, test_y = self._split_xy(data.iloc[validation_end:].reset_index(drop=True))
            return {
                "X_train": train_X,
                "y_train": train_y,
                "X_valid": valid_X,
        ...
        raise TypeError("DTRegressor data must be None, a dict, or a pandas DataFrame.")
...
if __name__ == "__main__":
    ...
    pipe = run_pipeline(source="local", fetch_new=False, enrich_details=True)
    train_df = load_dataset("train")
    val_df = load_dataset("validation")
    # test_df = load_dataset("test")
    # model_ready_df = load_dataset()
    # print(model_ready_df)
    # prediction_df = load_prediction_input()
    ...
    # lr = SGDRegressorScratch(X_train, y_train, max_iter=50000).train_and_evaluate(X_train, y_train, X_val, y_val)
    # lr2 = LinearRegressor(X_train, y_train, max_iter=50000).train_and_evaluate(X_train, y_train, X_val, y_val)
