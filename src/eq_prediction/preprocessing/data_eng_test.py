...
from eq_prediction.helpers.datapi import callDataFetcher
from eq_prediction.preprocessing.feature_eng import FilterAndFill
from eq_prediction.helpers.sql_data_handler import fetch_table

DATA_PATH = PROJECT_ROOT / "data" / "engineered_data" / "New_Engineered_Data.csv"
DATS_URL = "new_engineered_data"
...
def append_timestamp_to_file_name(file_name):
    ...
    if file_path.suffix:
        ...
        timestamped_name = f"{file_path.name}_{timestamp}"

    return str(file_path.with_name(timestamped_name))
...
def data_eng_test(data=None, max_workers=3, file_name=None):
    from eq_prediction.preprocessing.data_preprocessing import drop_rate_new
    if data is not None:
        df = data
    else:
        try:
            fetched_df = fetch_table("lappy_prepped_data", "new_engineered_data")
    ...
    output_file_name = append_timestamp_to_file_name(file_name or DATA_PATH.name)
...
def data_eng_test_incremental(
    ...
    if data is not None:
        ...
        try:
            ...
            if fetched_df is not None and not fetched_df.empty:
                ...
                if fetch_new:
                    print("Fetching fesh data from the Server.")
                df = callDataFetcher(fetch_new)
                print("Fetched saved data from the local")
        except Exception:
            if fetch_new:
                print("Fetching fesh data from the Server.")
            df = callDataFetcher(fetch_new)
    ...
    output_file_name = append_timestamp_to_file_name(file_name)
    output_path = _resolve_output_path(output_file_name)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    missing_data_df.to_csv(output_path, index=False)
    return missing_data_df
...
if __name__ == "__main__":
    ...
    print(data_eng_test_incremental(existing_file_name="New_Engineered_Data3.csv"))
