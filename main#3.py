...
if __name__ == "__main__":
    ...
    from src.helpers.datapi import callDataFetcher, DATA_PATH
    # data = pd.read_csv(DATA_PATH)
    dp = pd.read_csv(r"C:\Projs\COde\Earthquake\eq_prediction\data\eq_data_updated3.csv")
    dp['time'] = pd.to_datetime(dp['time'], unit='ms')
    dp.sort_values(by='time')
    dp.reset_index(inplace=True)
    dp.rename(columns={'index': 'idx'}, inplace=True)
    # dp.set_index("idx", inplace=True)
    print(dp.head(5))
    # print()
    print(dp.index)

    dp2 = pd.read_csv(r"C:\Projs\COde\Earthquake\eq_prediction\data\engineered_data\FinalRegressionData.csv")
    print(dp2.head(5))
    print(dp.index)
    # print(dp2.isna().sum())
    # print(f"{dp.shape}, {dp2.shape}")
    print()

    df = dp.combine_first(dp2)
    # print(df.shape)
    # print(df.columns)
    print(df.isna().sum())    
    print(df.head(10))
